...
def test_valid_csv_loads_correctly(tmp_path):
    # Contenido CSV con columnas y valores correctos para una fila
...
"""
    # Archivo temporal recibe el contenido CSV para el test
    file = tmp_path / "test.csv"
    file.write_text(content)

    # Instancia DataLoader y carga archivo CSV
    loader = DataLoader()
    df = loader.load(file)

    # Verifica que la salida es un DataFrame de pandas
    assert isinstance(df, pd.DataFrame)
    # Verifica que el DataFrame tiene 1 fila y 8 columnas (las esperadas)
    assert df.shape == (1, 8)
